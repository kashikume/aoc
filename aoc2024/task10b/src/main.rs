...
impl Map {
    ...
    fn calculate_score_for(&self, index: usize) -> i32 {
        ...
        check.insert(Trail{data:vec![index]});
        for value in 1..10 {
            let mut next_check = HashSet::new();
            for trail in check.iter() {
                let (x, y) = self.index_to_pos(*trail.data.last().unwrap());
                if self.get(x - 1, y) == value {
                    let mut new_trail = trail.clone();
                    new_trail.data.push(self.pos_to_index(x - 1, y));
                    next_check.insert(new_trail);
                }
                if self.get(x + 1, y) == value {
                    let mut new_trail = trail.clone();
                    new_trail.data.push(self.pos_to_index(x + 1, y));
                ...
                if self.get(x, y - 1) == value {
                    let mut new_trail = trail.clone();
                    new_trail.data.push(self.pos_to_index(x, y - 1));
                    next_check.insert(new_trail);
                }
                if self.get(x, y + 1) == value {
                    let mut new_trail = trail.clone();
