...
impl Robot {
    fn work(&self, keys:&String) -> Vec<String> {
        let mut outputs = Vec::new();
        let inpls: Vec<String> = if self.next_robot.is_none() {
            vec![keys.clone()]
        } else {
            let next_work = self.next_robot.as_ref().unwrap().work(keys);
            next_work.iter().cloned().collect()
        };
        for inp in inpls.iter() {
            let mut pos = self.keyboard.keys.get(&'A').unwrap().clone();
            let mut output = String::new();
            for key in inp.chars() {
                let new_pos = self.keyboard.keys.get(&key).unwrap();
                let mut dy = new_pos.1 - pos.1;
                let mut dx =  new_pos.0 - pos.0;

                while dx != 0 || dy != 0 {
                    let temp_pos_dy = (pos.0, pos.1 + dy);
                    if dy != 0 && !self.keyboard.emptys.contains(&temp_pos_dy) {
                        if dy < 0 {
                            output.push_str(&"^".repeat(dy.abs() as usize));
                        } else {
                            output.push_str(&"v".repeat(dy as usize));
                        }
                        dy = 0;
                        pos = temp_pos_dy;
                    }
                    let temp_pos_dx = (pos.0 + dx, pos.1);
                    if dx != 0 && !self.keyboard.emptys.contains(&temp_pos_dx) {
                        if dx < 0 {
                            output.push_str(&"<".repeat(dx.abs() as usize));
                        } else {
                            output.push_str(&">".repeat(dx as usize));
                        }
                        dx = 0;
                        pos = temp_pos_dx;
                    }
