...
impl Solution {
    ...
    fn check(&self, word: &String, start_x: i32, start_y: i32, dx: i32, dy: i32) -> bool {
        let mut x = start_x - dx * word.len() as i32 / 2;
        let mut y = start_y - dy * word.len() as i32 / 2; 
        for c in word.chars() {
            if c != self.get(x, y) {
                return false;
            }
    ...
    fn check_all(&self, word: &String) -> i32 {
        ...
        for y in 0..self.height {
            for x in 0..self.width {
                if (self.check(word, x, y, 1, 1) || self.check(word, x, y, -1, -1))
                &&  (self.check(word, x, y, -1, 1) || self.check(word, x, y, 1, -1)) {
                    out += 1;
                }
            }
        }
...
fn main() {
    ...
    let count = solution.check_all(&String::from("MAS"));
    println!("{}", count);
}
